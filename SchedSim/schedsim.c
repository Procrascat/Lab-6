...
{ 
    //Source: https://www.geeksforgeeks.org/program-for-shortest-job-first-sjf-scheduling-set-2-preemptive/
      /*
    ...
    int remainingTime[n];
    int currentProcess = 0, time = 0, minimum = INT_MAX;
    int smallest = 0, fTime;
    int check = 1;
    
    for(int bCount = 0; bCount < n; bCount++) {
      remainingTime[bCount] = plist[bCount].bt;
    }  
  
    
    while(currentProcess != n) {
      for(int j = 0; j < n; j++) {
        if((plist[j].art <= time) && (remainingTime[j] < minimum) && remainingTime[j] > 0) {
      ...
      minimum = remainingTime[smallest];
      if(minimum == 0) {
        minimum = INT_MAX;
      }
      ...
      if(remainingTime[smallest] == 0) {
        ...
        plist[smallest].wt = fTime - plist[smallest].bt - plist[smallest].art;
...
{ 
  ...
  int priResult;
  int result;
  const ProcessType *One = this;
  const ProcessType *Two = that;
  if(One->bt < Two->bt) {
    btResult = 1;
  }else{
    btResult = 0;
